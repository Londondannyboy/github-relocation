...
class EditorAgent {
  ...
  async prepareForPublication() {
    ...
    // Convert categories and tags to Sanity references
    const categoryRefs = await this.convertCategoriesToReferences(this.finalArticle.categories);
    const tagRefs = await this.convertTagsToReferences(this.finalArticle.tags);

    // Handle featured image if available
    const featuredImage = await this.processFeaturedImage();
    ...
    const article = {
      ...
      contentTier: 'tier1',
      generationCost: 0.02,
      readTime: Math.ceil(this.draft.metadata.wordCount / 200) // Fixed field name
    };
...
| Benefits | Full access | Limited |`;
  ...
  /**
   * Convert category strings to Sanity reference objects
   */
  async convertCategoriesToReferences(categories) {
    ...
    try {
        ...
        if (typeof categoryName === 'string') {
          ...
          if (categoryId) {
            ...
            const newCategory = await client.create({
              _type: 'category',
              title: categoryName,
              slug: { current: this.generateSlug(categoryName) },
      ...
      // Fallback to Digital Nomad category if available
      try {
        const fallbackCategory = await client.fetch(`*[_type == "category" && title == "Digital Nomad"][0]`);
        if (fallbackCategory) {
          categoryRefs.push({
            _type: 'reference',
            _ref: fallbackCategory._id,
            _key: crypto.randomUUID()
          });
        }
      } catch (fallbackError) {
        console.error('Error with fallback category:', fallbackError);
      }
    }
  ...
  /**
   * Convert tag strings to Sanity reference objects
   */
  async convertTagsToReferences(tags) {
    if (!tags || !Array.isArray(tags)) return [];
    
    const tagRefs = [];
    
    try {
      // Get existing tags from Sanity
      const existingTags = await client.fetch(`*[_type == "tag"]`);
      const tagMap = {};
      existingTags.forEach(tag => {
        const tagName = tag.title || tag.name; // Handle both title and name fields
        if (tagName) tagMap[tagName] = tag._id;
      });
        ...
        if (typeof tagName === 'string' && tagName.trim()) {
          ...
          if (!tagId) {
            ...
            const newTag = await client.create({
              _type: 'tag',
              title: tagName,
              slug: { current: this.generateSlug(tagName) }
            });
            tagId = newTag._id;
          }
  ...
  /**
   * Process featured image - upload to Sanity if available from enhanced copywriter
   */
  async processFeaturedImage() {
    try {
      // Check if images were generated by enhanced copywriter
      const images = this.draft.images || this.draft.metadata?.images;
      ...
      const asset = await client.assets.upload('image', buffer, {
        filename: `${this.generateSlug(this.finalArticle.title)}-featured.webp`,
        contentType: 'image/webp'
      });

      console.log('✅ Featured image uploaded:', asset._id);

      // Create SEO-optimized alt text based on focus keyword
      const focusKeyword = this.draft.metadata.primaryKeyword || this.finalArticle.title;
      const seoAltText = `${focusKeyword} guide - Requirements, costs, and application process explained`;

      // Return Sanity image object
      return {
        _type: 'image',
        alt: seoAltText,
        asset: {
